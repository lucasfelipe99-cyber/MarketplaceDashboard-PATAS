...
function statusFor(orderStatus, returnStatus, paidAt) {
  ...
  if ((key === 'cancelado' && text(paidAt) === '-') || key === 'nao pago') return 'Não pago';
...
function transformShopee(payload, pricingDatabase) {
  ...
  const output = rows.map((item) => {
    ...
    const commission = -(item.grossCommission + item.grossService) * share;
    ...
    const discount = -(directDiscount + cashDiscount) * share;
    const rebate = (item.campaignIncentive + item.campaignAdjustment + item.shopeeBundleDiscount) * share;
    const revenue = item.shopeeBundleDiscount * share + item.subtotal;
    const liquid = revenue + rebate + discount + commission + freight;
    const anticipation = liquid * -anticipationRate;
    const tax = -(revenue + discount) * taxRate;
    ...
    if (unitCost == null) missingCosts.add(costSku || '(SKU vazio)');
    const productCost = unitCost == null ? '' : -unitCost * item.quantity;
    const grossMargin = unitCost == null ? '' : liquid + tax + productCost + anticipation;
    const grossMarginPercent = unitCost == null || !revenue ? '' : grossMargin / revenue;
    ...
    return [
      ...
      statusFor(row[ix.orderStatus], row[ix.returnStatus], row[ix.paidAt]), text(row[ix.shippingOption]), fullSku,
      text(row[ix.productName]), costSku, item.agreedPrice, item.quantity, revenue, discount, rebate, commission,
      freight, item.reverseShipping, liquid, anticipation, tax, productCost, grossMargin, grossMarginPercent
    ];
  });
