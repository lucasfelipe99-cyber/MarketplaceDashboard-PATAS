...
function transformTikTok(payload, pricingDatabase) {
  ...
  const output = source.filter((row) => text(value(row, 'Order ID')) && !['Platform unique order ID.', 'x'].includes(text(value(row, 'Order ID')))).map((row) => {
    ...
    const liquid = revenue + commission + freight, tax = -taxRate * revenue, sku = text(value(row, 'Seller SKU'));
    const productCost = costResult(costs, sku, quantity, missing, status === 'Amostra Grátis');
    ...
    return ['TikTok', text(payload.channelName), text(value(row, 'Order ID')), value(row, 'Created Time'), date, orderStatus, status,
      ...
      revenue, 0, 0, commission, freight, 0, liquid, 0, tax, productCost, margin,
...
function transformAmazon(payload, pricingDatabase) {
  ...
  const output = sales.map((row) => {
    ...
    if (status!=='Venda') freight=0; else if (!Number.isFinite(freight)||freight===0) freight=revenue<78?-6.75:-19.50;
    const liquid=status==='Venda'?revenue+freight+commission+discount:0, tax=-taxRate*(revenue+discount), sku=amazonSku(rawSku);
...
function transformMagalu(payload, pricingDatabase, idMap) {
  ...
  const output=items.map((item)=>{const row=item.row,m=item.money,s=summaries.get(item.order),pack=packageMap.get(item.order)||{},statusRaw=text(value(pack,'Status pacote no momento que o relatório foi solicitado')),status=sold.has(statusRaw)?'Venda':'Cancelado';const visible=m['Pago pelo Parceiro (Coparticipação de Desconto à Vista)']-m['Pago pelo Magalu (Coparticipação de Desconto à Vista)']-m['Pago pelo Magalu (Valor subsídio Cupom)']+m['Pago pelo Parceiro (Valor subsídio Cupom)']-m['Pago pelo Magalu (Coparticipação de Preço Promocional)'];const difference=m['Valor bruto do pedido']-(s.total+s.visible),hidden=difference<=0&&s.promo?m['Pago pelo Parceiro (Coparticipação de Preço Promocional)']/s.promo*difference:0,discount=visible+hidden,rebate=m['Pago pelo Magalu (Coparticipação de Desconto à Vista)']+m['Pago pelo Magalu (Coparticipação de Preço Promocional)']+m['Pago pelo Magalu (Valor subsídio Cupom)'];const fee2Raw=norm(value(row,'Tarifa fixa (Forma de pagamento 2)')),fixed=m['Tarifa fixa (Forma de pagamento 1)']+(fee2Raw==='nao se aplica'?0:m['Tarifa fixa (Forma de pagamento 2)']),fixedAllocated=fixed===0?-5*item.quantity:fixed/s.count,variable=(s.total?item.fat/s.total:0)*m['Serviços do marketplace (1+2+3+4) (Forma de pagamento 1)'],commission=fixedAllocated+variable,eligible=m['Valor Total do Item']>78.99,freight=m['Coparticipação de Fretes estimada']!==0&&eligible&&s.eligible?m['Coparticipação de Fretes estimada']/s.eligible:0,liquid=item.fat+discount+rebate+commission+freight,tax=-taxRate*(item.fat+discount),code=text(value(row,'Codigo SKU seller')),sku=magaluSku(code,idMap),productCost=costResult(costs,sku,item.quantity,missing,false),margin=productCost===''?'':liquid+tax+productCost;return ['Magalu',text(payload.channelName),item.order,value(row,'Data do Pedido'),dateOnly(value(row,'Data do Pedido')),statusRaw,status,value(pack,'Modalidade de entrega'),code,value(row,'Título do produto'),sku,m['Valor Total do Item'],item.quantity,item.fat,discount,rebate,commission,freight,0,liquid,0,tax,productCost,margin,margin===''?'':item.fat?margin/item.fat:''];});
