(function () {
      ...
      Object.keys(metadata).forEach(function (monthKey) {
        ...
        if (dataset && Array.isArray(dataset.dataRows)) {
          dataset.dataRows.forEach(function (row) {
            ...
            if (normalizeText(category) !== normalizeText('13.Faturamento Bruto')) return;
            if (normalizeText(ad) === 'xx') return;
            revenue += parseNumber(row[dataset.columns.amount]) || 0;
          });
        }
        var previous = salesDre.months && salesDre.months[monthKey] ? salesDre.months[monthKey] : {};
        refreshedMonths[monthKey] = Object.assign({}, previous, { revenue: revenue });
      });
  ...
  function renderDre() {
    ...
    var revenue = activeMonths.length ? getDreActual('FATURAMENTO TOTAL', activeMonths[0], year) : 0;
    var margin = activeMonths.length ? getDreActual('MARGEM DE CONTRIBUIÇÃO TOTAL R$', activeMonths[0], year) : 0;
    var operating = activeMonths.length ? getDreActual('RESULTADO OPERACIONAL R$', activeMonths[0], year) : 0;
    var result = activeMonths.length ? getDreActual('RESULTADO R$', activeMonths[0], year) : 0;
    document.getElementById('dreContainer').innerHTML = '<div class="closing-shell">' +
      ...
      kpis([['Faturamento Jan', revenue], ['Margem contribuição Jan', margin], ['Resultado operacional Jan', operating], ['Resultado Jan', result]]) +
  ...
  function getDreActual(label, month, year) {
    ...
    var salesContribution = salesRevenue + salesCmv + salesTax + salesMkp;
  ...
  function renderBudget(selectedMonth) {
    var budgetYear = 2026;
    var activeMonths = Array.from(new Set(cashFlowState.records.filter(function (record) {
      return record.year === budgetYear;
    }).map(function (record) { return record.month; }).filter(Boolean))).sort(function (a, b) { return a - b; });
    var month = selectedMonth === 'accumulated' ? 'accumulated' : Number(selectedMonth) || activeMonths[0] || 1;
    ...
    var rows = details.map(function (row) {
      var budget = month === 'accumulated'
        ? sum((row.values || []).slice(0, activeMonths.length).map(function (value) { return value || 0; }))
        : ((row.values || [])[month - 1] == null ? 0 : row.values[month - 1]);
      var actual = getCashActualForBudgetLine(row.label, month, budgetYear);
      if (actual < 0 && budget > 0) budget = -budget;
      return { label: row.label, actual: actual, budget: budget, kind: row.kind, comment: row.comment || '' };
    });
    var body = rows.map(function (r) {
      ...
      return '<tr class="budget-' + (r.kind || 'detail') + '"><td>' + escapeCashHtml(r.label) + '</td><td>' +
        numericButton(r.label, month === 'accumulated' ? 13 : month, r.actual, 'orcamento') + '</td><td>' + money(r.budget) +
    ...
    var monthOptions = activeMonths.map(function (item) {
      return '<option value="' + item + '"' + (item === month ? ' selected' : '') + '>' + budgetState.months[item - 1] + '</option>';
    }).join('') + '<option value="accumulated"' + (month === 'accumulated' ? ' selected' : '') + '>Acumulado realizado</option>';
    document.getElementById('budgetContainer').innerHTML = '<div class="closing-shell">' +
      head('Orçado x Realizado', 'Estrutura e orçamento da planilha original; realizado recalculado pelo Fluxo de Caixa.',
        yearControl() + '<select class="closing-select" id="budgetMonth">' + monthOptions + '</select>') +
      '<div class="closing-card"><div class="closing-card-head"><h3>Comparativo do fechamento</h3><span class="closing-pill">' +
      (budgetState.sourceSheet ? 'Origem: ' + escapeCashHtml(budgetState.sourceSheet) : 'Aguardando importação do orçamento') +
      '</span></div><div class="closing-table-wrap"><table class="closing-table"><thead><tr><th>Linha DRE</th><th>Realizado</th><th>Orçado</th><th>Variação R$</th><th>Variação %</th><th>Atingimento</th><th>Status</th><th>Comentário</th></tr></thead><tbody>' + body + '</tbody></table></div></div></div>';
