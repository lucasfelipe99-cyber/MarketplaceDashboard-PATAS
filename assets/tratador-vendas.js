(function () {
  ...
  function historyHtml(channel){
    ...
    var groups=years.map(function(year){
      var cards=months.map(function(label,index){
        ...
        return '<div class="treat-history-item done"><b>'+label+' / '+year+'</b><span>'+Number(item.rowCount||0).toLocaleString('pt-BR')+' linhas</span><span>Datas no arquivo: '+formatDateBr(item.firstDate)+' a '+formatDateBr(item.lastDate)+'</span><small>Enviado em '+formatDateTimeBr(item.uploadedAt)+(item.sourceFile?' · '+esc(item.sourceFile):'')+'</small></div>';
      }).join('');
  ...
  document.addEventListener('click',async function(event){
    ...
    try{
      ...
      await recordTreatment({action:'record-treatment',id:channel.id,month:month.value,year:range.year,rowCount:rows.length-1,firstDate:range.firstDate,lastDate:range.lastDate,sourceFile:sourceFile,missingCostSkus:num(result.summary.missingCostSkus)});
      ...
      status.textContent=key!=='mercado livre'?(rows.length-1).toLocaleString('pt-BR')+' linhas processadas · '+num(result.summary.missingCostSkus).toLocaleString('pt-BR')+' SKU(s) sem custo · XLSX baixado e mês registrado.':(rows.length-1).toLocaleString('pt-BR')+' vendas tratadas para '+months[Number(month.value)-1]+' · mês registrado e resumo pronto para enviar à Base de Dados.';
