(function () {
  ...
  function repriceTreatedRows(rows,costMap,channel){
    ...
    var discountIndex=find(headers,['Desconto']);
    ...
    return [headers].concat(rows.slice(1).map(function(sourceRow){
      ...
      if(taxIndex>=0&&channel)row[taxIndex]=inactive?0:(-num(row[revenueIndex])-num(discountIndex>=0?row[discountIndex]:0))*(num(channel.taxRate)/100);
      ...
      for(var channelIndex=0;channelIndex<state.channels.length;channelIndex+=1){
        ...
        for(var recordIndex=0;recordIndex<records.length;recordIndex+=1){
          ...
          var result=await response.json();if(!response.ok)throw new Error(result.error||'Nao foi possivel recuperar o tratamento mensal.');
           var repriced=repriceTreatedRows(result.rows,costMap,channel),freightResult=await applyFreightAgreements(repriced),adjusted=freightResult.rows,range=treatmentRange(adjusted);
  ...
  function render() {
    ...
    form.addEventListener('submit',function(e){e.preventDefault();save({action:'upsert-channel',marketplace:e.target.marketplace.value,channelName:e.target.channelName.value,taxRate:num(e.target.taxRate.value),anticipationRate:num(e.target.anticipationRate.value),freight:num(e.target.freight.value),commissionBelowRate:num(e.target.commissionBelowRate.value),commissionBelowFixed:num(e.target.commissionBelowFixed.value),commissionAboveRate:num(e.target.commissionAboveRate.value),commissionAboveFixed:num(e.target.commissionAboveFixed.value),freightRate:num(e.target.freightRate.value)}).catch(function(err){alert(err.message);});});
    container.querySelectorAll('[data-channel]').forEach(function(card){var channel=state.channels.find(function(x){return x.id===card.dataset.channel;}),inputs=Array.from(card.querySelectorAll('[data-file-kind]')),treat=card.querySelector('[data-treat]'),publish=card.querySelector('[data-publish]'),scope=card.querySelector('[data-publish-scope]'),status=card.querySelector('.treat-status'),taxInput=card.querySelector('[data-tax-rate]'),saveTax=card.querySelector('[data-save-tax]');function ready(){var ok=inputs.every(function(input){return input.files[0];});treat.disabled=!ok;status.textContent=ok?'Relatórios prontos para tratar.':'Aguardando relatório.';}inputs.forEach(function(input){input.addEventListener('change',ready);});if(saveTax)saveTax.addEventListener('click',async function(){var taxRate=Number(taxInput.value);if(!Number.isFinite(taxRate)||taxRate<0||taxRate>100)return alert('Informe um imposto entre 0% e 100%.');var original=saveTax.textContent;try{saveTax.disabled=true;saveTax.textContent='Salvando...';await save({action:'update-tax-rate',id:channel.id,taxRate:taxRate});alert('Imposto atualizado. Use “Atualizar todas as bases” para recalcular os meses já tratados.');}catch(error){saveTax.disabled=false;saveTax.textContent=original;alert(error.message);}});if(publish)publish.addEventListener('click',async function(){var original=publish.textContent;try{publish.disabled=true;publish.textContent='Preparando...';status.textContent='Recuperando os meses tratados do disco...';var prepared=await rowsForPublish(channel,scope?scope.value:'all');if(!prepared.rows)throw new Error('Nenhuma linha tratada foi encontrada.');var name='RESUMO VENDAS E VARIAÇÃO - '+channel.channelName+' - '+prepared.labels.join(', ')+'.xlsx',staged=window.salesBaseIntegration.stageTreatedRows(prepared.rows,name);if(!staged)throw new Error('Não foi possível preparar a segunda etapa.');status.textContent=prepared.labels.length+' mês(es) enviados para Subir Base de Vendas: '+prepared.labels.join(', ')+' . Nenhum dado foi publicado ainda.';var destination=document.querySelector('[data-tab="salesUploadPanel"]');if(destination)destination.click();}catch(error){status.textContent=error.message;alert(error.message);}finally{publish.disabled=false;publish.textContent=original;}});card.querySelector('[data-delete]').addEventListener('click',function(){if(confirm('Excluir o canal '+channel.channelName+'?'))save({action:'delete-channel',id:channel.id}).catch(function(err){alert(err.message);});});});
  }
