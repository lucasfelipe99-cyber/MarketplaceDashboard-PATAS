(function () {
  ...
  async function transform(file, channel, selectedMonth) {
    ...
    Object.keys(groups).forEach(function (id) {
      ...
      children.forEach(function (item) {
        ...
        var tax = (-faturamento - discount) * (num(channel.taxRate) / 100), sku = String(r[ix.sku] || '').trim();
        var cmv = -(costMap[sku] || 0) * units, gm = liquid + tax + cmv, date = selectedDate(r[ix.date], selectedMonth);
        if (!date) throw new Error('Data inválida na linha ' + item.line + '.');
        output.push(['Mercado Livre',channel.channelName,String(r[ix.sale] || '').replace(/\.0$/, ''),r[ix.date],date,r[ix.state],statusType(r[ix.state]),r[ix.delivery],r[ix.ad],title,sku,price,units,faturamento,discount,rebate,fee,freight,cancellation,liquid,0,tax,cmv,gm,faturamento ? gm / faturamento : 0,id]);
      });
    });
  ...
  function downloadTreated(rows,channel,month){
    ...
    var suffix=norm(channel.marketplace)==='mercado livre'?months[Number(month)-1]:channel.marketplace;
  ...
  function render() {
    ...
    container.innerHTML='<div class="treat-shell"><div class="treat-head"><div><span>CONFIGURAÇÃO · VENDAS</span><h2>Tratador de Vendas</h2><p>Cadastre cada canal e trate os relatórios brutos no padrão do marketplace.</p></div><form id="treatChannelForm"><select name="marketplace">'+marketplaceOptions+'</select><input name="channelName" required placeholder="Nome do canal / empresa"><input name="taxRate" type="number" min="0" max="100" step=".01" value="14" required placeholder="Imposto %"><label class="treat-shopee-param" hidden><span>Antecipação %</span><input name="anticipationRate" type="number" min="0" max="100" step=".01" value="2.5"></label><label class="treat-shopee-param" hidden><span>Frete</span><input name="freight" type="number" step=".01" value="0"></label><button>Adicionar canal</button></form></div><section class="treat-list">'+(state.channels.length?state.channels.map(function(channel){var key=norm(channel.marketplace),direct=key!=='mercado livre';return '<article data-channel="'+channel.id+'" data-marketplace="'+esc(channel.marketplace)+'"><div class="treat-channel"><b>'+esc(channel.marketplace)+'</b><strong>'+esc(channel.channelName)+'</strong><small>Imposto: '+num(channel.taxRate).toLocaleString('pt-BR')+'%'+(key==='shopee'?' · Antecipação: '+num(channel.anticipationRate).toLocaleString('pt-BR')+'% · Frete: '+num(channel.freight).toLocaleString('pt-BR',{style:'currency',currency:'BRL'}):'')+'</small></div>'+(direct?'':'<label class="treat-month"><span>Mês da venda</span><select data-sale-month>'+months.map(function(m,i){return '<option value="'+(i+1)+'"'+(i===new Date().getMonth()?' selected':'')+'>'+m+'</option>';}).join('')+'</select></label>')+uploads(channel)+'<button data-treat disabled>'+(direct?'Tratar e baixar XLSX':'Tratar arquivo')+'</button>'+(direct?'':'<button class="primary" data-publish disabled>Enviar para Subir Base de Vendas</button>')+'<button class="danger" data-delete>Excluir canal</button><div class="treat-status">Aguardando relatório.</div></article>';}).join(''):'<div class="treat-empty">Adicione o primeiro canal de marketplace.</div>')+'</section></div>';
    ...
    container.querySelectorAll('[data-channel]').forEach(function(card){var channel=state.channels.find(function(x){return x.id===card.dataset.channel;}),month=card.querySelector('[data-sale-month]'),inputs=Array.from(card.querySelectorAll('[data-file-kind]')),treat=card.querySelector('[data-treat]'),publish=card.querySelector('[data-publish]'),status=card.querySelector('.treat-status');function ready(){var ok=inputs.every(function(input){return input.files[0];});treat.disabled=!ok;status.textContent=ok?'Relatórios prontos para tratar.':'Aguardando relatório.';}inputs.forEach(function(input){input.addEventListener('change',ready);});if(month)month.addEventListener('change',function(){if(preparedChannel===channel.id&&preparedMonth!==month.value){publish.disabled=true;status.textContent='Competência alterada. Trate o arquivo novamente.';}});if(publish)publish.addEventListener('click',function(){if(!preparedRows[channel.id]||preparedMonth!==month.value)return alert('Trate novamente o arquivo deste canal e competência.');var name='RESUMO VENDAS E VARIAÇÃO - '+channel.channelName+' - '+months[Number(month.value)-1]+'.xlsx',staged=window.salesBaseIntegration.stageTreatedRows(preparedRows[channel.id],name);if(!staged){status.textContent='Não foi possível preparar a segunda etapa.';return;}status.textContent='Resumo enviado para Subir Base de Vendas. Nenhum dado foi publicado ainda.';var destination=document.querySelector('[data-tab="salesUploadPanel"]');if(destination)destination.click();});card.querySelector('[data-delete]').addEventListener('click',function(){if(confirm('Excluir o canal '+channel.channelName+'?'))save({action:'delete-channel',id:channel.id}).catch(function(err){alert(err.message);});});});
  }
  document.addEventListener('click',async function(event){
    ...
    try{
      ...
      preparedRows[channel.id]=rows;preparedChannel=channel.id;preparedMonth=month?month.value:'';if(publish)publish.disabled=false;
      if(key!=='mercado livre')downloadTreated(rows,channel,'');
      status.textContent=key!=='mercado livre'?(rows.length-1).toLocaleString('pt-BR')+' linhas processadas · '+num(result.summary.missingCostSkus).toLocaleString('pt-BR')+' SKU(s) sem custo · XLSX baixado.':(rows.length-1).toLocaleString('pt-BR')+' vendas tratadas para '+months[Number(month.value)-1]+' · resumo pronto para enviar à Base de Dados.';
